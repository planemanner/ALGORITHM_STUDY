...
class Solution:
    def countPaths(self, n: int, roads: List[List[int]]) -> int:
        ...
        start_paths = adj_list[0]

        n_shortest_paths = 0
        shortest_length = float('inf')

        for a_path in start_paths:
            tmp_visit = {}
            search_stack = [a_path]
            tmp_time = 0
            while search_stack:
                node, time_cost = search_stack.pop()
                tmp_time += time_cost

                if node == (n-1):
                    if shortest_length > tmp_time:
                        n_shortest_paths = 1
                        shortest_length = tmp_time

                        break
                    elif shortest_length == tmp_time:
                        n_shortest_paths += 1
                    ...
                    for v, t in adj_list[node]:
                        ...
                        if v not in tmp_visit:
                            search_stack.append((v, t))
...
solver = Solution()
print(solver.countPaths(n=7, roads=[[0,6,7],[0,1,2],[1,2,3],[1,3,3],[6,3,3],[3,5,1],[6,5,1],[2,5,1],[0,4,5],[4,6,2]]))